...
pub fn close(fs: &mut dyn Fs, p: Lines) -> Union2<(), Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    return fs.close(p.s);
}

pub fn open_lines(fs: &mut dyn Fs, path: &String) -> Union2<Lines, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
...
pub fn close__2(fs: &mut dyn Fs, p: Chunks) -> Union2<(), Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    return fs.close(p.s);
}

pub fn open_chunks(fs: &mut dyn Fs, path: &String, size: i32) -> Union2<Chunks, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    let mut opened = fs.open_read(path);
    if matches!(opened, Union2::U2(_)) {
...
pub fn read_to_str(fs: &mut dyn Fs, path: &String) -> Union2<String, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    let mut opened = fs.open_read(path);
    if matches!(opened, Union2::U2(_)) {
...
pub fn read_lines(fs: &mut dyn Fs, path: &String) -> Union2<Vec<String>, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    let mut opened = fs.open_read(path);
    if matches!(opened, Union2::U2(_)) {
...
pub fn write_str(fs: &mut dyn Fs, path: &String, content: &String) -> Union2<i64, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
...
pub fn read_to_bytes(fs: &mut dyn Fs, path: &String) -> Union2<Vec<u8>, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    let mut opened = fs.open_read(path);
    if matches!(opened, Union2::U2(_)) {
...
pub fn write_bytes_to(fs: &mut dyn Fs, path: &String, data: &Vec<u8>) -> Union2<i64, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    let mut opened = fs.open_write(path);
    if matches!(opened, Union2::U2(_)) {
        return Union2::<i64, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>>::U2(opened.u2().clone());
    }
...
pub fn fill_from(fs: &mut dyn Fs, s: &InStream, buf: &mut Vec<u8>) -> Union2<i64, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
...
pub fn copy_stream(fs: &mut dyn Fs, s: &InStream, w: &OutStream) -> Union2<i64, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
    let mut buf = Vec::<u8>::new();
    let mut total: i64 = 0i64;
...
pub fn copy_file(fs: &mut dyn Fs, from: &String, to: &String) -> Union2<i64, Checked<Union8<NotFound, PermissionDenied, AlreadyExists, NotADirectory, PathEscapes, InvalidUtf8, StaleHandle, IoError>>> {
