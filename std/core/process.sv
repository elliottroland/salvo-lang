...
// here is `intrinsic` — a pid, a reply token and a pool are handles into the
...
// mints a [Pid], `replyto k(captures)` mints a [Reply], and `waitfor` is
...
// spawn hands back: the state behind it is the child's alone, so a `Pid` is
...
// bound to the same effect, so `use pid` and `use H()` are the same act on
// the calling side.
//
// A pid is freely copyable and never linear: sending to a process that has
...
// [linear-group].
//
// A `Reply<T>` is a pid with a single send member, so `send(r, v)` — or
