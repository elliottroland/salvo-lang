...
// Mutable variant
intrinsic fn mut_set_by<T>(size: Int, init: (Int) -> T) [] -> Mut Set<T>
    => size, init
...
intrinsic fn to_str<T>(set: Set<T>) [] -> Str => set
...
qualifier Distinct<T> of List<T> with NonEmpty, Sorted
...
intrinsic fn to_list<T>(set: Set<T>) [] -> List<T> as Distinct => set
...
intrinsic fn snapshot_at<T>(items: List<T>, index: Int) [] -> T? => items, index
...
// other.
fn iter<T>(set: Set<T>) [] -> Mut SetYield<T> => set {
...
// them [iter-protocol].
struct SetYield<T> : Yield<self, T> canbe Mut {
...
// set.
fn next<T>(p: Mut SetYield<T>) [] -> Emitted T | Finished => p: Mut {
