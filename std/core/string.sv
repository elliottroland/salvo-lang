...
intrinsic fn split(str: Str, sep: Str) [] -> List<Str> => str, sep
...
intrinsic fn index_of(str: Str, needle: Str) [] -> Int? => str, needle

// Whether [needle] occurs anywhere in [str]
intrinsic fn contains(str: Str, needle: Str) [] -> Bool => str, needle

// Whether [str] begins with [prefix]
intrinsic fn starts_with(str: Str, prefix: Str) [] -> Bool => str, prefix

// Whether [str] ends with [suffix]
intrinsic fn ends_with(str: Str, suffix: Str) [] -> Bool => str, suffix
...
intrinsic fn trim_prefix(str: Str, prefix: Str) [] -> Str => str, prefix

// [str] without a trailing [suffix] — unchanged when it does not end with
// one.
intrinsic fn trim_suffix(str: Str, suffix: Str) [] -> Str => str, suffix

// The characters of [str] from [start] (inclusive) to [end] (exclusive), or
// `None` when that range does not lie within the string.
intrinsic fn substr(str: Str, start: Int, end: Int) [] -> Str? => str, start, end
...
intrinsic fn join(parts: List<Str>, sep: Str) [] -> Str => parts, sep

// The integer [str] spells, or `None` when it does not spell one
intrinsic fn parse_int(str: Str) [] -> Int? => str
...
// `set` an `append`.
intrinsic fn set(str: Mut Str, index: Int, chr: Char) [] -> None => str: Mut, index, chr

// Removes every character from [str]
intrinsic fn clear(str: Mut Str) [] -> None => str: Mut
