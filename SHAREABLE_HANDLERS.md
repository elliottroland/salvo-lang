...
| SH-5 | `[waitfor]`: (a) keep step 1 whole / (b) optional **checked** annotation / (c) delete the word outright / (d) **move it to the boundary** | — | ✅ **decided (d) and built** (2026-09-19): the capability deleted whole, occupancy site-inferred with construction-propagation in the graph, the lambda refusal kept on its own reason, `thread()`/`Dedicated Pool` kept as chosen placement, `waitfor` in a list a parse error naming the deletion; the word returns at the host boundary with FC-7 (COMPLETED.md's log) |
| SH-9 | Direct-answer façades (§3.5) as a declared rung, and the default | as stated / collapse into SH-1 / omit | ✅ **decided and in force** (2026-09-19): a mixed handler is direct-answer unless a member declares `defer` — the opt-in carried by the declaration since the `defer` build's first half; the pumping request send stays deferred (bifurcation) |
...
| SH-10 | Marking the rung (§3.8) + the deferral deduction (§3.9) | M-1…M-5; (b) exact vs upper bound; (c) general vs Reply-specific | ✅ **decided and half-built** (2026-09-19): `defer` parses everywhere ([defer-deduction]), a move to callers, the rung-4 contract point enforced at four escape hooks, upper bound honored, **forwarding deferral runs end to end on both backends**. The remaining half is parking: the servant-continuation emission, precisely specified in ROADMAP |
| SH-7 | `use H() on POOL` sugar for `spawn` + `use addr` | yes / no | ✅ **decided yes and built** (2026-09-19): parsed as a `use` of a spawn expression, zero emitter changes; dep clauses wait for the `using` rename (COMPLETED.md's log) |
