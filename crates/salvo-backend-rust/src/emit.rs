...
impl<'p> Emitter<'p> {
    ...
    fn emit_struct(&mut self, s: &StructDecl) -> String {
        ...
        // generated member is defined in terms of them [rs-cmp-groups]. (This
        // used to read `canbe hashed`/`canbe ordered`, which the ordering round
        // deleted — the opt-in became the implementation.)
        let defaulted = |group: &str| {
            s.obligations
                .iter()
                .any(|o| o.default && o.group.name.name == group)
        };
        let hashed = defaulted("Hashed");
        let ordered = defaulted("Ordered");
    ...
    /// — so a canonical, a generated structural member [cmp-default] and an
