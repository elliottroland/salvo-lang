...
impl<'p> Emitter<'p> {
    ...
    fn emit_module(&mut self, module: &Module) -> String {
        ...
        for item in &module.items {
            match item {
                ...
                // [cmp-default] A structural member has no body and is still
    ...
    fn emit_struct(&mut self, s: &StructDecl) -> String {
        ...
        // [cmp-default] A `default` obligation asks for the same derive the
        // `canbe` opt-in does — the generated member is defined in terms of it
        // [rs-cmp-groups] — so the two spellings meet here.
        let defaulted = |group: &str| {
            s.obligations
                .iter()
                .any(|o| o.default && o.group.name.name == group)
        };
        let hashed = s.auto_qualifiers.iter().any(|q| q.name.name == "hashed") || defaulted("Hashed");
        let ordered =
            s.auto_qualifiers.iter().any(|q| q.name.name == "ordered") || defaulted("Ordered");
    ...
    /// [cmp-default] [rs-cmp-groups] A canonical generated by a `default`
    ...
    fn emit_fn_inner<'a>(&mut self, f: &FnDecl, style: FnStyle<'a>, indent: usize) -> String {
        // [cmp-default] A generated structural member has no body to emit: its
    ...
    fn rust_fn_name(&mut self, decl: &FnDecl) -> String {
        ...
        let overloads: Vec<&FnDecl> = match self.symbols.fns.get(name.as_str()) {
            // [cmp-default] A generated structural member has no body and is
            // still emitted, so it takes part in mangling like any overload —
            // without this, three `default Eq` structs would all emit `eq`.
            Some(o) if o.len() > 1 => o
                .iter()
