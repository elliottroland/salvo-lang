...
fn kotlinc_compiles_and_runs_a_qualifier_pick() -> KotlinCase {
    ...
    kotlin_case(files, "pick", PICK_OUTPUT)
}

fn kotlinc_compiles_and_runs_the_safe_call() -> KotlinCase {
    let program = build_program(&[("main.sv", SAFE_CALL_DEMO)]);
    let files = salvo_backend_kotlin::emit_program(&program).unwrap_or_else(|errors| {
        panic!("codegen errors:\n{}", errors.join("\n"));
    });
    kotlin_case(files, "safe-call", SAFE_CALL_OUTPUT)
}

fn kotlinc_compiles_and_runs_the_elvis_operator() -> KotlinCase {
    let program = build_program(&[("main.sv", ELVIS_DEMO)]);
    let files = salvo_backend_kotlin::emit_program(&program).unwrap_or_else(|errors| {
        panic!("codegen errors:\n{}", errors.join("\n"));
    });
    kotlin_case(files, "elvis", ELVIS_OUTPUT)
}
...
export fn by_len(a: Str, b: Str) [] -> Int => a, b {
...
/// Source and expected stdout are **verbatim** the Rust backend's
/// `rustc_compiles_and_runs_a_heap` (bar `export` on `main`). The exercise the
/// qualifier round was for, and the parity is the assertion: two heaps under two
/// orderings, drained by one function, printing the same thing on both targets.
fn kotlinc_compiles_and_runs_a_heap() -> KotlinCase {
    let src = r#"
...
/// (user decisions 2026-09-21, the ordering round's step 5).
///
/// Source and expected stdout are **verbatim** the Rust backend's
...
const KOTLIN_CASES: &[fn() -> KotlinCase] = &[
    ...
    kotlinc_compiles_and_runs_a_heap,
