...
pub fn emit_program_reporting(
    ...
    // [iter-effects] Where each effect's *interface* lives, as a
    ...
    // [iter-effects] One generated interface per effect set a claiming
    // producer (or a widening into one) needs, mapped to the Kotlin effect
    // types its `advance` takes as handler parameters.
    ...
    if !pass_traits.is_empty() {
        files.push(EmittedFile {
            rel_path: std::path::PathBuf::from("iter_effects.kt"),
...
/// [kt-union-wrappers].
/// [iter-effects] [kt-pass-effects] The generated support file for
/// **claiming** producers: one interface per effect set a program's producer
/// types mention, plus the factory that mints its passes and the adapter a
/// pure producer is widened with.
///
/// One interface per *set*, the way `unions.kt` gets one wrapper per arity — a
/// claiming pass cannot be a Kotlin `Iterator<T>`, because its `advance` takes
/// the handlers as parameters. The set is the checker's
/// (`Checked::pass_effect_sets`), so the parameter order here is the order the
/// machine and every drive site use.
///
/// `advance` reports a Boolean and leaves the element in `current()` rather
/// than returning `T?`, for the reason the whole protocol tags its end: a `T?`
/// return cannot tell "no more" from "the element is null" — the same choice
/// `SalvoPass` already made for the pure path.
///
/// The effect interfaces are named by **fully-qualified name**
/// (`salvo.core.console.Console`) rather than imported: this file lives in the
/// root `salvo` package and mentions interfaces from arbitrary module
/// packages, and the pure runtime file it sits beside has no cross-package
/// reference at all, so there is no import machinery to reuse and none is
/// needed.
fn generate_iter_effects_file(traits: &BTreeMap<Vec<String>, Vec<String>>) -> String {
    let mut out = String::from(
        "// Generated by the Salvo compiler: one pass interface per effect set \
         [iter-effects].\npackage salvo\n",
    );
    for (set, effects) in traits {
        ...
        let params: Vec<String> = set
            .iter()
            .zip(effects)
            .map(|(name, e)| format!("{}: {e}", effect_param_name(name)))
            .collect();
        let params = params.join(", ");
        out.push_str(&format!(
            "\n/// One pass whose driving performs {}.\ninterface SalvoPass{suffix}<T> {{\n    \
             /// Advance one element; `true` means `current()` holds it.\n    fun advance({params}): \
             Boolean\n    fun current(): T\n    /// The release path: pending deferred blocks, \
             latest first. Idempotent.\n    fun close({params})\n}}\n",
            set.iter()
                .map(|e| format!("`{e}`"))
                .collect::<Vec<_>>()
                .join(", ")
        ...
        out.push_str(&format!(
            "\n/// `{} Iter<T>`: a factory of those, so it stays replayable.\nfun interface \
             SalvoIter{suffix}<T> {{\n    fun mint(): SalvoPass{suffix}<T>\n}}\n",
            set.join(" ")
        ));
        out.push_str(&format!(
            "\n/// [iter-effects] The variance adapter: a producer performing *fewer* effects\n\
             /// fits where more are expected, and here that is a representation change,\n\
             /// so the compiler inserts this at the boundary.\nclass \
             SalvoPureAs{suffix}<T>(private val source: Iterable<T>) : SalvoIter{suffix}<T> {{\n    \
             override fun mint(): SalvoPass{suffix}<T> {{\n        val it = source.iterator()\n        \
             return object : SalvoPass{suffix}<T> {{\n            private var value: Any? = \
             null\n            override fun advance({params}): Boolean {{\n                if \
             (!it.hasNext()) {{\n                    return false\n                }}\n                \
             value = it.next()\n                return true\n            }}\n\n            \
             @Suppress(\"UNCHECKED_CAST\")\n            override fun current(): T = value as T\n            \
             override fun close({params}) {{}}\n        }}\n    }}\n}}\n"
        ));
    }
    out
}

/// [iter-effects] The name suffix of the generated interface/factory pair for
/// one effect set: the effect names, sanitized and concatenated, in the
/// canonical order the checker recorded (`{Console}` -> `SalvoPassConsole`).
...
struct Emitter<'p> {
    ...
    /// [iter-effects] Every effect set whose generated pass interface this
...
impl<'p> Emitter<'p> {
    ...
    /// them in — the one table shared by the interface, the machine and this
    /// call.
    ///
    /// The `close` goes in a `finally`, which is how `defer` is lowered here
        ...
        (
            format!(
                "{pad}val {place} = {subject}.mint()\n\
                 {pad}try {{\n\
                 {pad}while ({place}.advance({args})) {{\n\
                 {inner_pad}val {var} = {place}.current()\n"
            ),
            format!("{pad}}} finally {{\n{inner_pad}{place}.close({args})\n{pad}}}\n"),
        )
    }
    ...
    fn emit_module(&mut self, module: &Module) -> String {
        ...
        if !self.union_sizes.is_empty() || self.needs_iter || !self.pass_traits.is_empty() {
    ...
    fn emit_fn_inner(&mut self, f: &FnDecl, kw: &str, indent: usize, top_level: bool) -> String {
        ...
        // with `use`.
        // [iter-effects] A producer's effects belong to its *machine*, not to
    ...
    /// [yield-fn-origin] origin form — since the machine is identical; the
    /// returned body is empty for an origin, which has no function at all.
    fn emit_generator_flavored(
        &mut self,
        f: &FnDecl,
        elem: &str,
        ...
        let is_origin = pass_name.is_some();
        let pad = "    ".repeat(indent);
        ...
        let pass = match pass_name {
            ...
            None => format!("__Pass_{}", self.kotlin_fn_name(f)),
        };
        ...
        for field in &plan.fields {
            ...
            match &field.kind {
                ...
                FieldKind::Pass { subject, span } => {
                    let rendered = self.gen_elem_type(subject, *span);
                    // [iter-effects] A *claiming* inner producer would need its
        ...
        // [iter-effects] A claiming pass is not a `SalvoPass<T>` subclass — it
        // is not a Kotlin `Iterator` at all — so it holds the element itself.
        // `Any?` rather than `T`, for the reason the protocol tags its end: a
        // null element is an element like any other.
        if !claim_names.is_empty() || is_origin {
        ...
        let (supertype, advance_kw, close_kw) = if is_origin {
            // [yield-fn-origin] No supertype: the drive site knows the
            // concrete machine, so there is nothing to dispatch through and
            // no `SalvoPass` lookahead to inherit.
            (
                String::new(),
                format!("fun __advance({handler_sig})"),
                format!("fun __close({handler_sig})"),
            )
            ...
            (
                format!("SalvoPass{suffix}<{elem}>"),
                format!("private fun __advance({handler_sig})"),
                format!("private fun __close({handler_sig})"),
            )
        };
        // The origin machine is named by the drive site, which may be in
        // another module, so it is not `private`.
        let mut item = if is_origin {
            format!("\nclass {pass}{generic_decl}({}) {{\n", ctor.join(", "))
        } else {
            format!(
                "\nprivate class {pass}{generic_decl}({}) : {supertype} {{\n",
                ctor.join(", ")
            )
        };
        item.push_str(&props);
        ...
        if is_origin {
            ...
            // The interface side: `advance` reports and `current()` hands over,
            // because a `T?` return could not tell "no more" from "the element
            // is null".
            item.push_str(&format!(
                "\n    override fun advance({handler_sig}): Boolean = \
                 __advance({handler_call})\n\n    \
                 @Suppress(\"UNCHECKED_CAST\")\n    override fun current(): {elem} = __current as \
                 {elem}\n\n    override fun close({handler_sig}) {{\n        \
                 __close({handler_call})\n    }}\n"
            ));
        }
        item.push_str("}\n");
        ...
        let body = if is_origin {
            ...
            // The factory stays a factory, so the claim does not cost
            // replayability [iter-effects].
            let names = claim_names.clone();
            let _ = self.producer_repr(&names, elem);
            format!(
                "{pad}    return SalvoIter{suffix}<{elem}> {{ {pass}({}) }}\n",
                args.join(", ")
            )
        };
        (body, item)
    ...
    /// [kt-generator] [fn-iterator] The `Iter<T>`-returning form.
    fn emit_generator(&mut self, f: &FnDecl, elem: &str, indent: usize) -> (String, String) {
        self.emit_generator_flavored(f, elem, None, indent)
    }
    ...
    fn emit_origin_machine(&mut self, f: &FnDecl) -> String {
        ...
        let (_, item) = self.emit_generator_flavored(f, &elem, Some(&machine), 0);
        item
    }
    ...
    fn emit_named_type(&mut self, qualifiers: &[TypeRef], base: &TypeRef) -> String {
        ...
        // so it cannot be a Kotlin `Iterator` and its factory is a type of its
        // own.
    ...
    /// [iter-effects] The effect claims of a written type's qualifier list, in
    /// canonical order: the qualifiers that name an **effect** rather than a
    /// qualifier declaration (the checker's precedence — a qualifier
    /// declaration of that name wins).
    ///
    /// A *generic* effect claim is refused rather than rendered: the set
    /// identity here would be this backend's rendering of the arguments while
    /// the checker's is Salvo's, and two spellings of one set would generate
    /// two interfaces that fit neither [backend-never-wrong].
    ...
    /// [iter-effects] The Kotlin type of a claiming producer: the generated
    /// factory for its effect set, registered so `iter_effects.kt` gets the
    /// interface it names.
    ...
    fn kotlin_ty(&mut self, ty: &Ty) -> String {
        match ty {
            ...
            Ty::Qualified { quals, base } => {
                // [iter-effects] An effect claim on a producer names the
                // generated factory for its effect set, so it is a different
                // *type*, not a forgotten claim.
    ...
    fn emit_ty(&mut self, ty: &Ty) -> String {
        match ty {
            ...
            Ty::Qualified { quals, base } => {
                // [iter-effects] A claiming producer is its own type here too.
                let claims = ty.effect_claims();
                if !claims.is_empty() {
                    if let Ty::Named { name, args } = base.as_ref() {
                        if name == "Iter" {
    ...
    fn emit_expr_stmt(&mut self, expr: &Expr, indent: usize) -> String {
        ...
        match expr {
            ...
            Expr::For {
                ...
                self.loop_results.pop();
                out.push_str(&format!("{pad}}}\n"));
                // [iter-effects] The injected `close`, in a `finally` so that
    ...
    fn apply_coercion_value(&mut self, coercion: Coercion, code: String) -> String {
        match coercion {
            ...
            // [iter-effects] The variance adapter: a pure producer wrapped for
            // a claiming position. The displaced change (a union wrap, say)
            // applies to the *widened* value, so it goes outside.
    ...
    /// [iter-effects] [kt-pass-effects] Wraps a producer for a position that
    ...
    fn widen_producer(&mut self, from: &Ty, to: &Ty, code: String) -> String {
        ...
        // Registers the interface/factory/adapter trio the call needs.
        let _ = self.producer_repr(&names, &elem);
        format!("SalvoPureAs{}({code})", pass_suffix(&names))
    }
...
/// A rendered type turned into an identifier fragment for a generated name
/// (`Random<Int>` -> `Random_Int`) [iter-effects].
