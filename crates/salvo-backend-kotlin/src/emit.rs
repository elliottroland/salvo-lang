...
pub fn emit_program(program: &Program) -> Result<Vec<EmittedFile>, Vec<String>> {
    ...
    // [kt-abort-signal] Generated once for the whole program, when
    // anything aborts.
    ...
    for (file_idx, unit) in program.units().enumerate() {
        ...
        needs_abort |= emitter.needs_abort;
    ...
    if needs_abort {
        files.push(EmittedFile {
            rel_path: std::path::PathBuf::from("abort.kt"),
            content: generate_abort_file(),
        });
    }
...
///
/// The `tag` is the Salvo type of the message as written at the abort site.
...
fn generate_abort_file() -> String {
    String::from(
        "// Generated by the Salvo compiler: the abort control signal.\n\
         package salvo\n\
         \n\
         class AbortSignal(val payload: Any?, val tag: String) :\n\
         \x20   RuntimeException(null, null, false, false)\n",
    )
}

/// Whether an effect instance is the abort effect [abort]: the JVM unwinds
/// to the delimiter, so it is never a handler parameter [kt-abort-signal].
...
struct Emitter<'p> {
    ...
    /// [kt-abort-signal] This file aborts (or delimits an abort), so the
...
impl<'p> Emitter<'p> {
    ...
    fn emit_fn_inner(&mut self, f: &FnDecl, kw: &str, indent: usize, top_level: bool) -> String {
        ...
        if effect_params_wanted {
            ...
            match checked_effects {
                Some(tys) => {
                    for ty in tys {
                        // [kt-abort-signal] Aborting needs no handler: the
                        // JVM unwinds to the innermost `try`, so `Abort`
    ...
    // ================= abort and `try` [kt-abort-signal] =================

    /// `try { ... }` [kt-abort-signal]: Kotlin's own `try`/`catch`, which is
    ...
    /// the `Aborted` arm. Catching the innermost signal is exactly
    ...
    fn emit_try(&mut self, body: &Block, span: Span) -> String {
        ...
        format!(
            "try {{\n{inner}{pad}}} catch (__signal: AbortSignal) {{\n\
             {inner_pad}{aborted}\n{pad}}}"
        )
    }
    ...
    fn emit_call(&mut self, callee: &Expr, type_args: &[Type], args: &[Expr], span: Span) -> String {
        // [abort] [kt-abort-signal] `abort(message)` is the control
        // transfer itself: a throw the innermost `try` catches. A call that
        // merely *propagates* an abort needs nothing — the JVM unwinds.
        if let Some(site) = self.checked.may_abort.get(&(self.file_idx, span)).cloned() {
            if site.performs {
                ...
                // [kt-abort-signal], since a throwing frame cannot know
                // which `try` will catch it.
                let tag = format!("{}", site.message);
                return format!("throw AbortSignal({message}, \"{tag}\")");
            }
        }
    ...
    fn emit_fn_call(
        ...
        match self.checked.call_effects.get(&(self.file_idx, span)).cloned() {
            Some(effs) if effs.iter().all(ty_is_concrete) => {
                for ty in &effs {
                    // [kt-abort-signal] Not a capability: nothing to pass.
                    if is_abort_effect_ty(ty) {
                        continue;
                    }
