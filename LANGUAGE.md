...
A deduction clause can also say *which field* a call mutates. `=> e: Mut` means "mutated somewhere in `e`", so everything derived from `e` is invalidated; `=> e.hp: Mut` says the mutation lands on that field alone, and a value derived from a different field — `let rings = e.rings` — survives the call. `=> !e.rings` is the other half: the field itself is **replaced**, so whatever was derived from it is gone. A written field entry is checked against the body, since every caller's precision rests on it.
